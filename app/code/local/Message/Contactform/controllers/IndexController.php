...
class Message_Contactform_IndexController extends Mage_Contacts_IndexController
{
    ...
    $getSession = Mage::getModel('core/session')->getContactvalidate();
    	//echo $getSession; exit;
    	if ($getSession)
		{
			if($getSession != 'validated')
			{
				Mage::getModel('core/session')->addError(Mage::helper('contactform')->__('Your Captcha is not validated'));
				$this->_redirectReferer();
				return;
			}else
			{
				Mage::getModel('core/session')->setContactvalidate('');
			}
		}
		else
		{			
		...
		if($captch=$_POST['defaultReal']){
			if ($this->rpHash($_POST['defaultReal']) != $_POST['defaultRealHash']) {
				Mage::getModel('core/session')->addError(Mage::helper('contactform')->__('The security code entered was incorrect. Please try again!'));
				//$this->_redirect('*/ /*');
				$this->_redirectReferer();
				return;
			}
