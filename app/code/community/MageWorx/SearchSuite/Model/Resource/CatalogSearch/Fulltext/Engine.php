...
class MageWorx_SearchSuite_Model_Resource_CatalogSearch_Fulltext_Engine extends MageWorx_SearchSuite_Model_Resource_CatalogSearch_Fulltext_Engine_Abstract {
    ...
    public function prepareResultForEngine($fulltextModel, $queryText, $query) {
        ...
        if (count($indexWords) == 0 && count($allWords) > 0) { /* search without fulltext index */
            ...
            $where = '(' . join(' OR ', $likes) . ')';
            $fields = $this->getResultFields('no_fulltext', $query, array('no_fulltext' => $ifcase));
            $select = $select
...
//                    }
            ...
            $fields = $this->getResultFields('fulltext', $query);
            $select = $select
...
//                    }
            ...
            $fields = $this->getResultFields('like_fulltext', $query);
            $select = $select
                    ->from(array($mainTableAlias => $adapter->select()->union($union)), $fields)
                    ->joinInner(array('e' => $fulltextModel->getTable('catalog/product')), 'e.entity_id = s.product_id', array())
                    ->group('product_id');
        ...
        if ($queryPrepared) {
            if ($this->checkCatalogSearchVersion() && $this->checkMagentoVersion()) {
                $this->foundData = $adapter->fetchPairs($select, $bind);
            } else {
    ...
    protected function getResultFields($condition, $query, $params = array())
    {
        $fields = array(
        ...
        if ($condition == 'no_fulltext') {
            $fields['relevance'] = new Zend_Db_Expr(join(' + ', $params['no_fulltext']));
        }
    ...
    protected function checkCatalogSearchVersion()
    {
        return version_compare(Mage::getConfig()->getModuleConfig('Mage_CatalogSearch')->version, '1.8.2.0', 'ge');
    }

    protected function checkMagentoVersion()
    {
        return version_compare(Mage::getVersion(), '1.9.3.0', 'ge');
    }
