...
/**
	...
	public function drawBuyerInfo($page, $order, $xoffset, $startY) {
		...
		$page -> drawText(Mage::helper('sales') -> __('Bill To') . ':', $xoffset, $infoY, 'UTF-8');
		$infoY -= 22;
		...
		if ($order -> getBillingAddress() -> getCompany() != '') {
			...
			if ($fullName != '')
			{
				...
				$infoY -= 10;
				Mage::helper('studio4_invoiceconfig/common')->drawIconOnPage($page, $xoffset, $infoY, 'company');
	...
	public function drawSellerInfo($page, $order, $xoffset, $startY) {
		...
		/* If shipping is different from billing info, add info that differs */
		if ($this -> params['showShippingAddress'] == 1) {
			$nameDiffers = 
				$order -> getBillingAddress() -> getFirstname() != $order -> getShippingAddress() -> getFirstname() || 
				$order -> getBillingAddress() -> getLastname() != $order -> getShippingAddress() -> getLastname();
			...
			//{
				...
				$this -> _setFontRegular($page, 10);	
				//$infoY -= 15;
				$page -> drawText(Mage::helper('sales') -> __('Ship To:'), $xoffset, $infoY, 'UTF-8');
				$infoY -= 22;
				$page -> setFillColor(new Zend_Pdf_Color_Html('#'.$this -> params['textColor']));
				$font = $this -> _setFontRegular($page, 16);
				...
				//if ($nameDiffers || true) {
					...
					$nameStr = strip_tags(ltrim($order -> getShippingAddress() -> getFirstname() . ' ' . $order -> getShippingAddress() -> getLastname()));
					...
					foreach ($commonHelper->sliceStringByPoints(mb_strtoupper($nameStr, 'UTF-8'), 244, $font, 16) as $_value) {
						$page -> drawText(trim($_value), $xoffset, $infoY, 'UTF-8');
						$infoY -= 16;
					}
