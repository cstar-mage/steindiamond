...
//                exit;
    ...
    public function showSwatchTitle($_value)
    {
        $return = '';
        if ($this->getShowSwatchTitle()) {
            $return .= '<div>'.$_value->getTitle().'</div>';
            if ($_value->getPriceType() == 'percent' || $_value->getPriceType() == self::PRICE_TYPE_OPTIONS_PERCENT) {
                $return .= '<div>'.number_format((float)$_value->getPrice(), 2, '.', '').'%'.'</div>';
            } else {
                $return .= '<div>'.Mage::helper('core')->currency($_value->getPrice(), true, false).'</div>';
            }
        }
        return $return;
    }
