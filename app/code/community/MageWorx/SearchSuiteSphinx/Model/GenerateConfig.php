...
class MageWorx_SearchSuiteSphinx_Model_GenerateConfig {
    ...
    private function getDbConnectionConfig()
    {
        ...
        $dbConnection = $config->getResourceConnectionConfig("default_setup");

        $dbInfo = array(
            '{$sql_host}' => $dbConnection->host->__toString(),
            '{$sql_user}' => $dbConnection->username->__toString(),
            '{$sql_password}' => $dbConnection->password->__toString(),
            '{$sql_database_name}' => $dbConnection->dbname->__toString(),
            '{$sql_port}' => $this->helper->getSqlPort(),
            '{$table_prefix}' => $config->getTablePrefix()->__toString()
        );
