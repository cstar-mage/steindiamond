...
abstract class MageWorx_SearchSuite_Model_Resource_Fulltext_Abstract extends Mage_Core_Model_Mysql4_Abstract {
    ...
    public function rebuildIndex($storeId = null, $ids = null) {
        ...
        if ($ids !== null) {
            $this->_rebuildIndividual($storeId, $ids);
        } else {
            $this->_rebuildAll($storeId);
        }
    ...
    protected function _rebuildAll($storeId)
    {
        if (is_null($storeId)) {
            $storeIds = array_keys(Mage::app()->getStores());
            foreach ($storeIds as $storeId) {
                $this->_rebuildStoreIndexForAll($storeId);
            }
        } else {
            $this->_rebuildStoreIndexForAll($storeId);
        }
    ...
    protected function _rebuildIndividual($storeId, $ids)
    {
        if (is_null($storeId)) {
            $storeIds = array_keys(Mage::app()->getStores());
            foreach ($storeIds as $storeId) {
                $this->_rebuildStoreIndexForIndividual($storeId, $ids);
            }
        } else {
            $this->_rebuildStoreIndexForIndividual($storeId, $ids);
        }
