...
class Ideal_Diamondsearch_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    public function listAction()
    {
		...
		*/
		...
		if(isset($post["shapes"]))
		{
			...
			*/
			//changed login for searching SuperNova
			if(!in_array("All", explode(",", $post["shapes"]))){
				$shapes = explode(",", $post["shapes"]);
				for($i=0;$i<count($shapes);$i++)
				{
					$likeShapes[] = array('like' => "%".$shapes[$i]."%");
				}
				$diamondsCollection->addFieldToFilter('shape', $likeShapes);
			}else{
				$shapes = $shapesArray;
				for($i=0;$i<count($shapes);$i++)
				{
    ...
    public function listcompareAction()
    {
		...
		if(isset($_COOKIE["compare_params"])) {
			$diamonds_id = array_filter(explode(",",$_COOKIE["compare_params"]));
		} 
		$diamondsCollection->addFieldToFilter('id', array('in' => $diamonds_id));
	...
	public function listrapnetAction()
	{
		$shapeConfig = unserialize(Mage::getStoreConfig("diamondsearch/shape_settings/shape_available"));
    	//FOR PHP 5.3 usort($shapeConfig, function($a, $b) {return $a['sortorder'] - $b['sortorder'];});
    	$shapesLabelArray = array();
		...
		$page_size = (int)$post["limit"];
		$page_number = ((int)$post["offset"]/(int)$page_size)+1;
		$total_pages = 0;
		$search_type = "White";
		...
		$request_json["request"]["body"]["search_type"]=$search_type;
		$request_json["request"]["body"]["page_number"]=$page_number;
		$request_json["request"]["body"]["page_size"]=$page_size;
		
		if(isset($post["shapes"]))
		{
			if(!in_array("All", explode(",", $post["shapes"])))
				$request_json["request"]["body"]["shapes"] = explode(",", $post["shapes"]);
			//else
			//	$request_json["request"]["body"]["shapes"] = $shapesArray;
		}
		if(isset($post["min_carat"]) && isset($post["max_carat"]) )
		{
			$request_json["request"]["body"]["size_from"] = $post['min_carat'];
			$request_json["request"]["body"]["size_to"] = $post['max_carat'];
		}
		if(isset($post["min_price"]) && isset($post["max_price"]) )
		{
			$request_json["request"]["body"]["price_total_from"] = $post['min_price'];
			$request_json["request"]["body"]["price_total_to"] = $post['max_price'];
		}
		...
		if(isset($post["colors"]) && $post["is_fancy"] == 0)
		{
			...
			$request_json["request"]["body"]["color_from"] = reset($likeColors);
			$request_json["request"]["body"]["color_to"] = end($likeColors);
		}
		elseif(isset($post["fancycolors"]) && $post["is_fancy"] == 1)
		{ 
			$request_json["request"]["body"]["fancy_colors"] = Mage::helper("diamondsearch")->getShortCodes("fancycolor", explode(",", $post["fancycolors"]));
			//$diamondsCollection->addFieldToFilter('fancy_intensity', array('in' => Mage::helper("diamondsearch")->getShortCodes("fancycolor", explode(",", $post["fancycolors"])) ));
		}
		if(isset($post["clarities"]))
		{
			$request_json["request"]["body"]["clarity_from"] = reset(explode(",", $post["clarities"]));
			$request_json["request"]["body"]["clarity_to"] = end(explode(",", $post["clarities"]));
		}
		if(isset($post["cuts"]) && count($cutsArray) != count(explode(",", $post["cuts"])))
		{
			$request_json["request"]["body"]["cut_from"] = reset(explode(",", $post["cuts"]));
			$request_json["request"]["body"]["cut_to"] = end(explode(",", $post["cuts"]));
		}

		if(isset($post["fluorescences"]) && count($fluorescencesArray) != count(explode(",", $post["fluorescences"])))
		{
			$request_json["request"]["body"]["fluorescence_intensities"] = Mage::helper("diamondsearch")->getShortCodes("fluorescence", explode(",", $post["fluorescences"]));
			//$diamondsCollection->addFieldToFilter('fluorescence', array('in' => Mage::helper("diamondsearch")->getShortCodes("fluorescence", explode(",", $post["fluorescences"])) ));
		}
		/*
		if(isset($post["min_ratio"]) && isset($post["max_ratio"]) )
		...
		if(isset($post["certificates"]))
		{
			...
			for($i=0;$i<count($certificates);$i++)
			{
				if($certificates[$i] == "Non-Certified")
				{
					$likeCertificates[] = "NONE";
					//$likeCertificates[] = array('like' => "");
				}
				else
					$likeCertificates[] = $certificates[$i];
			}
			$filtered_likeCertificates = array_filter($likeCertificates);
			if(!empty($filtered_likeCertificates))
				$request_json["request"]["body"]["labs"] = array_filter($likeCertificates);
		}
		else
		{
			$certificates = $certificatesArray;
		...
		if($orderby == "carat")$orderby = "Size";
		if($orderby == "totalprice")$orderby = "Price";
		if($orderby == "certificate")$orderby = "Lab";
		if(!in_array(strtolower($orderby), array("price","shape","size","color","clarity","cut","lab")))$orderby = "Size";
		...
		$request_json["request"]["body"]["sort_by"]  = $orderby;
		$request_json["request"]["body"]["sort_direction"]  = $orderMethod;
		
		//echo json_encode($request_json);
		//echo "<pre>";print_r($request_json); 
		//exit;
		
		$response = Mage::helper('diamondsearch/rapnet')->getRapnetInstantInvAPIResponse($user,$passwd,json_encode($request_json),$page_number,$page_size,$search_type);
		
		$result = array();
		if($response["response"]["header"]["error_code"]==0){
			//echo "<pre>";print_r($response); exit;
			$diamonds = Mage::helper("diamondsearch")->getRapnetDiamondsArray($response, $post["is_fancy"], $post["is_specialshape"]);
			$result = array("total"=>$response["response"]["body"]["search_results"]["total_diamonds_found"]);
			$result["rows"] = $diamonds;
		}else{
	...
	public function listrecentrapnetAction()
	{
		$diamonds_id = Mage::getSingleton('core/session')->getRecentlyViewed();
		...
		$diamLimit = 20;
		foreach($diamonds_id as $diamond_id){
			if($diamLimit-- == 0) break;
			...
			if($response["response"]["header"]["error_code"]==0){
				$diamonds[] = Mage::helper("diamondsearch")->getRapnetSingleDiamondArray($response, $_REQUEST["is_fancy"], $_REQUEST["is_specialshape"]);
			}
		}
		array_map("intval", $diamonds);
		//$total_count = (count($diamonds) > 20) ? 20 : count($diamonds);
    ...
    public function listcomparerapnetAction()
    {
		$diamonds_id = array();
		$compare_params = array();
		if(isset($_COOKIE["compare_params"])) {
	...
	public function listrecentrapnetdiamonddetailAction()
	{
		$diamonds_id = Mage::getSingleton('core/session')->getRecentlyViewed();
