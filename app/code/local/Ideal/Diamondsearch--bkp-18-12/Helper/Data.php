...
class Ideal_Diamondsearch_Helper_Data extends Mage_Core_Helper_Abstract
{
	function getDiamondsArray($stones, $is_fancy, $is_specialshape)
	{
		...
		foreach ( $stones as $stone) {
			...
			if(in_array($stone['lotno'], $images_flag)) $diamond_image = $stone['diamond_image'];
				...
				if(file_exists(Mage::getBaseDir('media'). DS . "dsearch/samples" . DS .strtolower($this->getSpecialAbbrnew($stone['shape']))."_pic_large.jpg")){
					$diamond_image = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . DS . "dsearch/samples" . DS .strtolower($this->getSpecialAbbrnew($stone['shape']))."_pic_large.jpg";
					$is_samp = true;
				}
			...
			*/
			...
			$cut_data = $this->getFullNameFromAbbreviation("cut", strtoupper($stone['cut']));
			if((!($show_cut_if_its_n) && $cut_data != "" && $cut_data != "None" && $cut_data != "N") || ($show_cut_if_its_n && $cut_data != "" && $cut_data != "None" && $cut_data != "N")){
				$cut = $cut_data;
			}
			elseif(!($show_cut_if_its_n) && ($cut_data == "" || $cut_data == "None" || $cut_data == "N")){
				$cut = "";
			}
			elseif($show_cut_if_its_n && ($cut_data == "" || $cut_data == "None" || $cut_data == "N")){
					$cut = $cut_data;
			...
			$diamonds[] = array(
				...
				"depth" => number_format($stone['depth'], 1),
				"tabl" => number_format($stone['tabl'], 1),
	...
	function getRapnetDiamondsArray($stones, $is_fancy, $is_specialshape)
	{
		$diamonds = array();
		$compare_params = array();
		if(isset($_COOKIE["compare_params"])) {
		...
		foreach ($stones["response"]["body"]["diamonds"] as $stone) {
			$currencySym = $stone['currency_symbol'];
			...
			if(!$cfp_data["is_cfp"])
				$price = number_format(round((float)$stone['total_sales_price']));
			else
				$currencySym = "";
			
			//$link = $this->getDiamondLink($stone['size'],$stone['shape'],$stone['cut'],$stone['lab'],$color,$stone['clarity'],$stone['stock_num'],$stone['cert_num']);
			$link = Mage::getUrl("diamondsearch/index/view", array("id"=>$stone['diamond_id']));

			$diamond_image = false;
			$is_samp = false;
			if(in_array($stone['stock_num'], $images_flag)) $diamond_image = $stone['diamond_image'];
			else {
			...
			$diamonds[] = array(
				"compare" => (in_array($stone['diamond_id'],$compare_params)?true:false),
				"id" => (int)$stone['diamond_id'],
				"lotno" => $stone['stock_num'],
				"shape" => $shape,
				"carat" => number_format((float)$stone['size'], 2, '.', ''),
				"color" => strtoupper($color),
				"clarity" =>$stone['clarity'],
				"cut" => $cut,
				"fluorescence" => strtoupper(trim($stone['fluor_intensity'])),
				"ratio" => '',
				"certificate" => $stone['lab'],
				"inhouse" => '',
				"totalprice" => $currencySym.$price,
				"percent_rap" => '',
				"availability" => '',
				"dimensions" => $stone["meas_length"]."x".$stone["meas_width"]."x".$stone["meas_depth"],
				"depth" => $stone["depth_percent"],
				"tabl" => $stone["table_percent"],
				"polish" => strtoupper($stone['polish']),
				"symmetry" => strtoupper($stone['symmetry']),
				"culet" => strtoupper($stone['culet_size']),
				"cert_number" => $stone['cert_num'],
				"crown" => '',
				"pavilion" => '',
				"girdle" => strtoupper($stone['girdle_min']."-".$stone['girdle_max']),
				"make" => '',
				"country" => '',
				"image" => false,
				"is_samp" => $is_samp,
	...
	function getRapnetSingleDiamondArray($stones, $is_fancy, $is_specialshape)
	{
		//CALL for price
		$cfp_data = $this->getCallForPriceData();
		$price = "CALL";
		...
		$stone = $stones["response"]["body"]["diamond"];
		$currencySym = $stone['currency_symbol'];
	...
	function getInitSearchPanelDataJson()
	{
		...
		*/
		...
		if($diamondsCollection->getSize()){
			...
			$respArray["min_carat"] = round(floor($mainColl[0]["min_carat"]),2);
			$respArray["max_carat"] = round(ceil($mainColl[0]["max_carat"]),2);
			$respArray["min_totalprice"] = round(floor($mainColl[0]["min_totalprice"]));
			$respArray["max_totalprice"] = round(ceil($mainColl[0]["max_totalprice"]));
			$respArray["min_ratio"] = round(floor($mainColl[0]["min_ratio"]),2);
			$respArray["max_ratio"] = round(ceil($mainColl[0]["max_ratio"]),2);
		}
	...
	public function resizeImage($imageName, $width=NULL, $height=NULL, $imagePath=NULL)
	{
		...
		//if (file_exists($imagePathFull) && !file_exists($resizePathFull)) { - Removed by Anil. causes issue if already image exists
