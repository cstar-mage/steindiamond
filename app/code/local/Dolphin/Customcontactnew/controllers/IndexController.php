...
class Dolphin_Customcontactnew_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    {
		 if($captch=$_POST['defaultReal99']){
    if ($this->rpHash($_POST['defaultReal99']) != $_POST['defaultReal99Hash']) {
    	...
    	if ($post) {
    		...
    		for($i = 0; $i <= 6; $i++){
    			try {
    				...
    				$post['image'.$i] = "media/special_order/".$post['email']."/".$_FILES['image_'.$i]['name'];
    		...
    		try {
    		 ...
    		 $mailTemplate->setDesignConfig(array('area' => 'frontend'))
    			...
    			$model = Mage::getModel('customcontactnew/customcontactnew')->setData($post);
    			if(!$model->save()){
    				throw new Exception();
    			}
    ...
    /*
    		...
    		$translate->setTranslateInline(true);
            Mage::getSingleton('customer/session')->addSuccess('Your Message was send Successfully');
    		$this->_redirect('custom-designs-page');
