 ...
 */
class Studio4_InvoiceConfig_Model_Bundle_Sales_Order_Pdf_Items_Creditmemo extends Mage_Bundle_Model_Sales_Order_Pdf_Items_Abstract
{
    /**
     * Draw item line
     *
    ...
    public function draw()
    {
		...
		foreach ($items as $_item) {
			...
			if ($this->canShowPriceInfo($_item)) {
                $itemDetails['rowTotalEx'] = $order->formatPriceTxt($_item->getRowTotal());
				$itemDetails['discount'] = $order->formatPriceTxt(-$_item->getDiscountAmount());
				$itemDetails['qty'] = $_item->getQty()*1;
				$itemDetails['tax'] = $order->formatPriceTxt($_item->getTaxAmount());
                $itemDetails['rowTotalInc'] = $order->formatPriceTxt(
	...
	public function drawParent($details, $returnHeight = false)
	{
		...
		$nameParts = $commonHelper->sliceStringByPoints($details['name'], 156, $font, 9);
		...
		//slice SKU
		//$skuParts = Mage::helper('core/string')->str_split($details['sku'], 21);
		$skuParts = $commonHelper->sliceStringByPoints($details['sku'], 100, $font, 9);
		$skuOffset = 0;
		...
		if (!$returnHeight && !empty($details['rowTotalEx']))
		{
			$font = $pdf->_setFontBold($page, 9);
			$page->drawText($details['rowTotalEx'], 304, $y, 'UTF-8');
		}
		
		if (!$returnHeight && !empty($details['discount']))
		{
			$font = $pdf->_setFontBold($page, 9);
			$page->drawText($details['discount'], 360, $y, 'UTF-8');
		}
		...
		if (!$returnHeight && !empty($details['rowTotalInc']))
		{
			$font = $pdf->_setFontBold($page, 9);
			$page->drawText($details['rowTotalInc'], $pdf->getAlignRight($details['rowTotalInc'], $page->getWidth() - 60 - 28, 60, $font, 9, 0), $y, 'UTF-8');
		}
	...
	public function drawOptionLabel($details, $returnHeight = false)
	{
		...
		//slice product name into chunks
		//$nameParts = Mage::helper('core/string')->str_split('• ' . $details['name'], 33, true, true);
		$nameParts = $commonHelper->sliceStringByPoints('• ' . $details['name'], 154, $font, 9);
	...
	public function drawItem($details, $returnHeight = false)
	{
		...
		//slice product name into chunks
		//$nameParts = Mage::helper('core/string')->str_split($details['name'], 32, true, true);
		$nameParts = $commonHelper->sliceStringByPoints($details['name'], 141, $font, 8);
