...
{	
	...
	{
		$filename = Mage::getBaseDir('var').'/import/diamonds.csv';
		if(file_exists($filename)){
			copy($filename, sys_get_temp_dir()."/diamonds.csv");
			header("Content-type: text/csv");
			header("Content-disposition: attachment; filename = diamonds.csv");
			readfile(sys_get_temp_dir()."/diamonds.csv");
		}else{
			echo "No diamonds CSV found";
		}
	} 
	public function downloadListRapnetAction()
	{
		$filename = Mage::getBaseDir('var').'/import/rapnet/rapnet.csv';
		if(file_exists($filename)){
			copy($filename, sys_get_temp_dir()."/rapnet.csv");
			header("Content-type: text/csv");
			header("Content-disposition: attachment; filename = rapnet.csv");
			readfile(sys_get_temp_dir()."/rapnet.csv");
		}else{
			echo "No rapnet CSV found";
		}
	} 
	public function downloadListRapnetInstantAction()
	{
		$filename = Mage::getBaseDir('var').'/import/rapnet/rapnet_inst.csv';
		if(file_exists($filename)){
			copy($filename, sys_get_temp_dir()."/rapnet_inst.csv");
			header("Content-type: text/csv");
			header("Content-disposition: attachment; filename = rapnet_inst.csv");
			readfile(sys_get_temp_dir()."/rapnet_inst.csv");
		}else{
			echo "No rapnet instant CSV found";
		}
	} 
	...
	public function getRapnetInstantInvListAction()
	{
		$importCSV = Mage::helper('uploadtool')->getRapnetInstantInvList();
		if($importCSV['success'] == 0) {
			Mage::getSingleton("adminhtml/session")->addError($importCSV['message']);
			$this->_redirect("*/*/new");
		} else {
	...
	public function updateRapnetInstantInvDiamondsAction()
	{
		$saveCSV = Mage::helper('uploadtool')->updateRapnetInstantInvDiamonds();
		if($saveCSV['success'] == 0) {
			Mage::getSingleton("adminhtml/session")->addError($saveCSV['message']);
			$this->_redirect("*/*/new");
		} else {
